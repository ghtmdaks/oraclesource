...
-- 서브 쿼리 : sql 문을 실행하는데 필요한 데이터를 추가로 조회하기 위해 sql문
--          내부에서 사용하는 select 문을 말해;
--          연산자와 같은 비교 또는 조회 대상의 오른쪽에 놓여서 괄호로 묶어서 사용;
--          서브쿼리의 SELECT 절에 명시한 열의 메인쿼리의 비교 대상과 
--          같은 자료형과 같은 개수로 지정;
...
--ANY(SOME) : 메인쿼리의 조건식을 만족하는 서브쿼리의 결과가 하나 이상이면 TRUE 크다작다 할 때 유용해
...
-- 굳이 MIN 이나 MAX를 뽑지 않아도 돼
--  30번 부서 사원들의 최고급여보다 많은 사원 조회
SELECT * FROM EMP WHERE SAL > ANY(SELECT SAL FROM EMP WHERE DEPTNO=30);
...
--묹제] 전체 사원 중 ALLEN 과 같은 직책인 사원들의 사원정보, 부서정보 출력하기
...
-- 출력(급여가 많은 순으로 정렬하되 급여가 같은 경우에는 사원번호를 오름차순으로 정렬)
SELECT ENAME, EMPNO, JOB, SAL, D.DEPTNO, D.DNAME 
FROM EMP E, DEPT D 
...
-- 40번 부서의 부서이름과 지역을 수정하고 싶어(DEPT 테이블의 DEPT =30인 지역명, 부서명 추출)
SELECT DNAME, LOC
...
INSERT INTO EXAM_EMP VALUES(7202,'TEST-USER2',CLERK,7201,'2016-02-21',4500,NULL,50);
INSERT INTO EXAM_EMP VALUES(7203,'TEST-USER3',ANALYST,7201,'2016-04-11',3400,NULL,60);
INSERT INTO EXAM_EMP VALUES(7204,'TEST-USER4',SALESMAN,7201,'2016-05-31',2700,300,60);
...
INSERT INTO EXAM_EMP VALUES(7208,'TEST-USER8',STUDENT,7201,'2018-03-09',1200,NULL,80);
...
--TRANSACTION
--하나의 단위로 데이터를 처리하겠다! 계좌이체 생각하면 이해하기 편해
...
--       데이터베이스 접속을 시작으로 여러 데이터 베이스에 관련된 작업을 수행한 후
--       접속을 종료하기까지 전체 기간
